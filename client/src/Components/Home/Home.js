...
function Home() {
    ...
    useEffect(() => {
        async function FetchNews() {
            setIsLoading(true)
            const response = await axios.get(`https://newsapi.org/v2/everything?q=${"all"}&from=${date.getFullYear()}-${date.getMonth()}-${date.getDate()}&to=${date.getFullYear()}-${date.getMonth()}-${date.getDate()}&sortBy=popularity`)
            ...
            <div className="container">
                ...
                {/* <input className="input" id="search" placeholder="Search here: Biden"/> */}
                        <div class="form-floating mb-3">
                            ...
                            <div className="sbtn"><button className="s-btn" onClick={searcHandler}><i class="fas fa-solid fa-magnifying-glass">Search</i></button></div>
                        </div>
                {news.map((newsItem) => (
